...
public class MapNewMarker extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_map_new_marker);

        final LatLng markerPos = getIntent().getParcelableExtra("mapPos");
        collapsingToolbarLayout = (CollapsingToolbarLayout) findViewById(R.id.collapsing_toolbar_marker);
        collapsingToolbarLayout.setCollapsedTitleTextColor(Color.parseColor("#FFFFFF"));
        collapsingToolbarLayout.setExpandedTitleColor(Color.parseColor("#8A000000"));
        ...
        markeradd_agency.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                new MaterialDialog.Builder(MapNewMarker.this)
                        .title("Select Agency")
        ...
        if (getIntent().getBooleanExtra("isMarkerVisible", false)) {
        ...
        Thread t = new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    matches = geoCoder.getFromLocation(markerPos.latitude, markerPos.longitude, 1);
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            locationField.setText(bestMatch.getAddressLine(0) + ", " + bestMatch.getLocality() + ", " + bestMatch.getAdminArea());
                            locationField.setPrimaryColor(Color.parseColor("#607D8B"));
                        }
                    });
                    Log.d(TAG, "LOCATION: " + bestMatch.toString());
                } catch (IOException e) {
                    e.printStackTrace();
                }

                final Bitmap b = BitmapFactory.decodeByteArray(getIntent().getByteArrayExtra("map"), 0, getIntent().getByteArrayExtra("map").length);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
        ...
        header = (ImageView) findViewById(R.id.map_image);
        ...
        for (final MapMarker mapMarker : markers.getMarkers()) {
            adapter.add(new MaterialSimpleListItem.Builder(MapNewMarker.this)
        ...
        coords.setText(String.valueOf(markerPos.latitude).substring(0, 9) + ", " + String.valueOf(markerPos.longitude).substring(0, 10));
        ...
        doneFab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (validateFields()) {
                    ...
                    alertValues.put("lat", markerPos.latitude);
                    alertValues.put("lon", markerPos.longitude);
                    ...
                    String key = firebaseDatabase.getReference().child("markers").push().getKey();

                    Map<String, Object> childUpdates = new HashMap<>();
                    childUpdates.put("/markers/" + key, alertValues);
                    childUpdates.put("/departments/" + departmentKeys.get(selectedDepartment) + "/markers/" + key, true);

                    firebaseDatabase.getReference().updateChildren(childUpdates);

                    if (selectedMarker.equals(Markers.hydrant)) {
                        saveHydrantValues(key);
                    } else {
                        setResult(RESULT_OK);
                        finish();
                    }
    ...
    private void loadHydrantOptions() {
        ...
        hydrantOutlets.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new MaterialDialog.Builder(MapNewMarker.this).title("Outlets")
        ...
        hydrantType.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new MaterialDialog.Builder(MapNewMarker.this).title("Hydrant Type")
                        .items(hydrant_types)
